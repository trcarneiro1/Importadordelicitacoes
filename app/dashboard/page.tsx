...
export default function DashboardPage() {
  ...
  const filteredLicitacoes = licitacoes.filter(lic => {
    if (filter.sre && !lic.sre_source.includes(filter.sre)) return false;
    if (filter.modalidade && lic.modalidade !== filter.modalidade) return false;
    if (filter.situacao && lic.situacao !== filter.situacao) return false;
    if (filter.valorMin && (!lic.valor_estimado || lic.valor_estimado < parseFloat(filter.valorMin))) return false;
    if (filter.valorMax && (!lic.valor_estimado || lic.valor_estimado > parseFloat(filter.valorMax))) return false;
    return true;
  });

  // Estatísticas
  const totalLicitacoes = filteredLicitacoes.length;
  const totalValor = filteredLicitacoes.reduce((sum, lic) => sum + (lic.valor_estimado || 0), 0);
  ...
  const licitacoesPorSRE = Object.entries(
    filteredLicitacoes.reduce((acc, lic) => {
      const sre = lic.sre_source.replace('.educacao.mg.gov.br', '').replace('sre', 'SRE ');
      acc[sre] = (acc[sre] || 0) + 1;
      return acc;
  ...
  const valorPorSRE = Object.entries(
    filteredLicitacoes.reduce((acc, lic) => {
      const sre = lic.sre_source.replace('.educacao.mg.gov.br', '').replace('sre', 'SRE ');
      acc[sre] = (acc[sre] || 0) + (lic.valor_estimado || 0);
      return acc;
    }, {} as Record<string, number>)
  )
  ...
  const licitacoesPorCategoria = Object.entries(
    filteredLicitacoes.reduce((acc, lic) => {
      const cat = lic.categoria || 'Outros';
      acc[cat] = (acc[cat] || 0) + 1;
      return acc;
    }, {} as Record<string, number>)
  ).map(([name, value]) => ({ name, value }));
  ...
  const sresUnicas = Array.from(new Set(licitacoes.map(l => l.sre_source))).sort();
  const modalidadesUnicas = Array.from(new Set(licitacoes.map(l => l.modalidade))).sort();
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredLicitacoes.slice(0, 50).map((licitacao) => (
                  <tr 
                    key={licitacao.id} 
                    onClick={() => window.location.href = `/dashboard/${licitacao.id}`}
